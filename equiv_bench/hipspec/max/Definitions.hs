...
ta1_fold :: (a -> a -> a) -> [a] -> a -> a
ta1_fold f l a =
    case l of
        [] -> a
        [hd] -> hd
        hd:tl -> f hd (ta1_fold f tl a)
...
ta1 lst =
    case lst of
        [] -> 0
        [hd] -> hd
        hd:tl -> if hd > ta1 tl then hd else ta1 tl

sol118 :: [Nat] -> Nat
sol118 lst =
    let 
        loop fir [] -> []
        loop fir (h:t) -> if h> (loop fir t) then h else loop fir t
    in
    case lst of
        [] -> 0
        hd:tl -> loop h t

sol164_fold :: (a -> a -> a) -> [a] -> a -> a
sol164_fold f l a =
    case l of
        [] -> a
        hd:tl -> f hd (sol164_fold f tl a)

sol164 :: [Nat] -> Nat
sol164 lst =
    case lst of
        [] -> 0
        hd:tl -> sol164_fold (\x y -> if x > y then x else y) lst hd
