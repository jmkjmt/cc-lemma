...
ta1 :: (Nat -> Bool) -> [Nat] -> [Nat]
ta1 pred [] = []
ta1 pred (hd:tl) | pred hd = hd : ta1 pred tl
                 | otherwise = ta1 pred tl

sol121 :: (Nat -> Bool) -> [Nat] -> [Nat]
sol121 pred lst = 
    let 
        loop [] o = o
        loop (h:t) o = loop t (if pred h then h:o else o)
...
sol171 :: (Nat -> Bool) -> [Nat] -> [Nat]
sol171 pred lst =
    let
        aux i [] = []
        aux i (h:t) = if pred h then h : aux (S i) t else aux (S i) t
    in
        aux (S Z) lst